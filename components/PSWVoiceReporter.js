...
const PSWVoiceReporter = () => {
  ...
  useEffect(() => {
    ...
    scrollToBottom();
  }, [selectedLanguage, browserSupport, handleSpeechInput]);
  ...
  }, [conversation, consecutiveAIMessages, generateReport, isProcessing, playAudio, selectedLanguage]);
  ...
  // Tailored Care Solutions Logo Component tuned to match the mock
  const TailoredCareLogo = ({ orientation = 'stacked' }) => {
    const inline = orientation === 'inline';
    const logoGradientId = useMemo(
      () => `logo-stem-${Math.random().toString(36).slice(2)}`,
      []
    );
    return (
      <div
        className={`flex ${inline ? 'flex-row items-center gap-5' : 'flex-col items-center gap-6'}`}
        style={{ color: brandColors.textGold }}
      >
        <div className="relative">
          <div
            className="w-24 h-24 rounded-[26px] border border-white/40 shadow-[0_20px_30px_rgba(178,120,42,0.35)] flex items-center justify-center bg-gradient-to-br from-[#FFF3DD] to-[#DCA75B]"
            style={{
              boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.4),0 25px 40px rgba(180,112,20,0.35)'
            }}
          >
            <svg viewBox="0 0 64 64" className="w-12 h-12" aria-hidden="true">
              <path
                d="M32 48c-6 0-9-5-9-12 0-11 8-18 8-18s8 7 8 18c0 7-3 12-7 12z"
                fill={`url(#${logoGradientId})`}
              />
              <path
                d="M26 29c-7-1-12-7-13-15 8 1 15 6 17 13"
              ...
              />
              <path
                d="M38 29c7-1 12-7 13-15-8 1-15 6-17 13"
                stroke="rgba(255,255,255,0.6)"
                strokeWidth="2.5"
                strokeLinecap="round"
                fill="none"
              />
              <defs>
                <linearGradient id={logoGradientId} x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor="#FFE6B3" />
                  <stop offset="100%" stopColor="#C57A1F" />
                </linearGradient>
              </defs>
            </svg>
          </div>
          <div className="absolute -inset-2 rounded-[32px] bg-gradient-to-r from-white/20 to-transparent blur-2xl opacity-70 pointer-events-none" />
        </div>

        <div className={`flex flex-col ${inline ? 'items-start text-left' : 'items-center text-center'}`}>
          {['Tailored', 'Care', 'Solutions'].map((word, index) => (
            <span
              key={word}
              className={`tracking-wide ${index === 0 ? 'text-3xl' : 'text-3xl'} font-semibold drop-shadow-sm`}
              style={{
                textShadow: '0 8px 20px rgba(181,112,18,0.35)',
                letterSpacing: index === 0 ? '0.08em' : '0.05em'
              }}
            >
              {word}
            </span>
  ...
  if (showReport) {
    return (
      <div className="min-h-screen" style={{ background: brandColors.champagne }}>
        <div className="max-w-5xl mx-auto px-4 py-10">
          <TailoredCareLogo />
          
          <div className="mt-10 bg-white rounded-3xl shadow-[0_30px_80px_rgba(20,8,2,0.08)] p-8 border border-[#F1E0CC]">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="px-6 py-2 rounded-full text-sm font-semibold text-[#2f1301] transition-all"
                  style={{
                    background: 'linear-gradient(135deg, #FFDDB2, #F2A347)',
                    boxShadow: '0 12px 30px rgba(227,162,72,0.35)'
                  }}
                >
                  New Session
                </button>
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    style={isSendDisabled
                      ...
                      : {
                          background: 'linear-gradient(135deg, #FFE4C1, #F2A247)',
                          boxShadow: '0 12px 25px rgba(227,162,72,0.35)'
                        }}
                  >
                    Send
                  </button>
                  ...
                  <div
                    ...
                    style={{
                      background: isUserMessage
                        ? 'linear-gradient(140deg, #F7BA6A, #D9852C)'
                        : '#FFF7EB',
                      border: isUserMessage ? 'none' : '1px solid rgba(194,122,31,0.2)',
                      boxShadow: isUserMessage
                        ? '0 12px 28px rgba(217,133,44,0.35)'
                        : '0 8px 20px rgba(32,21,5,0.05)'
                    }}
                  >
                      ...
                      {isTooLong && (
                        <button
                          ...
                          style={{
                            color: isUserMessage ? '#FFFFFF' : brandColors.textGold
                          }}
                        >
            ...
            {conversation.length > 0 && (
              <div className="mt-4 flex gap-2 justify-center">
                <button
                  ...
                  className="px-4 py-2 rounded-full font-medium transition-all hover:shadow-lg text-sm flex items-center gap-2"
                  style={{
                    backgroundColor: brandColors.lightGold,
                    color: brandColors.darkBlue,
                    border: '1px solid rgba(194,122,31,0.2)'
                  }}
                >
            ...
            {conversation.length > 1 && !isReportGenerating && (
              <div className="mt-6 flex gap-3">
                ...
                <button
                  ...
                  className="px-4 py-3 border rounded-full font-medium transition-all"
                  style={{
                    borderColor: 'rgba(14,21,53,0.25)',
