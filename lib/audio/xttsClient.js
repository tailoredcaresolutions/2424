...
export class XTTSClient {
  constructor(options = {}) {
    this.model = options.model || process.env.XTTS_MODEL || 'tts_models/multilingual/multi-dataset/xtts_v2';
    this.device = options.device || process.env.XTTS_DEVICE || 'mps'; // Metal acceleration
    this.sampleRate = parseInt(options.sampleRate || process.env.XTTS_SAMPLE_RATE || '24000');
...
`;
  ...
  getSupportedLanguages() {
    return [
      { code: 'en', name: 'English (Canadian)', flag: '🇨🇦' },
...
const xttsClientInstance = new XTTSClient();
export default xttsClientInstance;
